...
class Profile_Widget extends WP_Widget {
    ...
    */
    function __construct() {
        parent::__construct(
            'profile_widget', 
            __( 'Profile Download Widget', 'TEXTDOMAIN' ), // Name
            array( 'description' => __( 'Use this widget to display and setup the display of your profile download link', 'TEXTDOMAIN' ), ) // Args
        );
    }
    ...
    */
    public function widget( $args, $instance ) {
        ...
        if( NULL != $profile_link_url ) :
            ...
            // If no file uploaded, and user is logged in and admin
            if( current_user_can( 'edit_others_pages' ) ) :
                echo $args[ 'before_widget' ];
                echo $args[ 'before_title' ] . apply_filters( 'widget_title', __( 'File Missing!', 'TEXTDOMAIN' ) ) . $args[ 'after_title' ];
                /*TODO Fix customiser link to go directly to "File Uploads" section and autofocus on the Company Profile upload control*/
                printf( __( '%sThis widget is not setup correctly. The linked file does not exist. Please upload one %s here %s', 'TEXTDOMAIN' ), '<p>', '<a href="' . admin_url( 'customize.php' ) . '">', '</a>', '</p>' );
                echo $args[ 'after_widget' ];
            else :
                return;
            endif;
